...
优先级：P2
关联：ADR-0001（命名策略：Unix 短名 + 语义化别名）、ADR-0011（别名保留给 MCP 侧）、**ADR-0016（四层优先级链——本工单取舍依据）**
...
7. **裁决依据（ADR-0016）**：新增别名落实链第 3 项（短名省工具名 token），但**不损害链第 4 项（输入 token）**——别名是 AI 需记忆的额外词汇，克制边界（7 个封顶、冲突护栏钉死遮蔽风险）即此裁决。`callTool` 收敛到 `findTool` 消除双实现，避免 AI 因坏特性试错多走一轮（链第 1 项）。
8. **与 12 号白名单的解析一致性**：`callTool` 复用 `findTool` 后，若 `findTool` 查询注入后的工具表（受 `WIN_SHELL_TOOLS` 过滤），则被裁别名/正名与 `batch_run` 一致返回「未在当前部署暴露」而非 `Unknown tool`；若查询全局表则不一致。实施时确认 `findTool` 基于注入工具表，确保与 12 号白名单过滤语义对齐。
