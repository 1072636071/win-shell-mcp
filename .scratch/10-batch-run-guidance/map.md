...
- **裁决依据**：ADR-0016——本批次服务链第 1 项（轮速最少：多步任务一次完成、减少多轮往返），省 token 是兑现后的连带收益；与 08（链第 4 项）合并编辑时按链裁决冲突。
...
- **已解决（2026-08-26）**：01/02 全部 resolved。最终描述恰 150 字符、四段式齐备，`batch_run` 移出 `DESCRIPTION_EXCEPTIONS`；护栏新增引导语义 4 断言（优先/一次 + 读文件/替换/写回 + 长度护栏）。
- **偏差**：① 描述以「本工具」自指（非字面 batch_run）换取压进 150 软上限——工具名在 ListTools 与描述相邻呈现，无歧义；② 场景省略「grep 定位」环节，保留最小流程 读文件→替换→写回。均记录于工单 01 实施记录。
- **联动**：09 号 outputSchema 超集增长使护栏基线重取为 50516（非本批次主动收紧）。
