...
优先级：P1
关联：ADR-0006（成域闸门，15 命令域）、ADR-0011（全量注册）、ADR-0007（兼容性红线）、**ADR-0016（四层优先级链——本工单取舍依据）**
...
4. **护栏测试**（沿用 `guard-mutating.test.ts` 模式）：每个工具 `domain` 非空；15 个域每个至少一个工具；域计数总和 + 3 个 meta = **61**（现状 59 = 58 域工具 + `batch_run` 1 meta；本工单新增 `tool_groups`/`list_domain_tools` 两 meta → 落地后 58 域 + 3 meta = 61）；CONTEXT.md 基线数（15 域/59 工具）作为常量写入护栏并注释来源，基线更新时须同步改。
...
- **裁决依据（ADR-0016）**：本机制落实链第 4 项（输入 token 最少：砍 59 工具固定开销），以一次链第 2 项（LLM 请求）成本换取。按链规则「下游项是上游项让步对象」，第 4 项是第 2 项下游、为下游增益牺牲上游成本**违反链方向**——缓解：该导航往返归入任务开环的同一用户轮内（仅第 2 项内部请求、第 1 项轮速不变），且调用不设门禁 + `batch_run` 恒可见保住链第 1 项（见实现决策 5/用户故事 4）。此为「一次性第 2 项成本换结构化、可复现的第 4 项收益」的刻意覆盖，故要求实施时先验证客户端行为（发布门槛），若客户端禁止调用未列出工具则降级为受限特性。
