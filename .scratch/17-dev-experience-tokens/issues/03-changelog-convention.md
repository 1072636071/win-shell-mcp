...
- **2026-08-26 实施完成**：README 开发章节（"构建产物为多入口 ESM…"之后）新增「CHANGELOG 维护约定」小节——条目从 `git log`（自上个版本标签起，仓库尚无标签时以最近一次版本提交为基线）的 commit message 汇总提炼，按 Added/Changed/Fixed 分类、补 ADR/工单交叉引用、不要求回顾全量 diff；配套要求 commit message 自带完整主题与引用。仅加文档，不触碰 src/、不改测试。
