...
DSH 官方 4 模式（标准 / PTC / 极简 / 创造）、dsh-liangshen 插件（梁神模式）与
本项目 3 模式的提示词文本库（每份提示词一个英文生效版 + 一个中文阅读版）见
[提示词/README.md](提示词/README.md)。

系统提示的**装填原理、流水线与各模式装填完毕后的形态**（含一次 Web GUI 实测
样本的逐段解剖）见 [装配原理与形态.md](装配原理与形态.md)。
