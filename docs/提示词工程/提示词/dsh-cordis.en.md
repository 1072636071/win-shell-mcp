...
来源：`deepseek-harness/packages/preset/agent-presets/presets/cordis/agent.cordis.yml` 的 `persona` 行（`text: |-`，五段）。该 preset 未开 `complete: true`，同样拼 harness 固定 opener。
...
骨架与 [dsh-standard.en.md](dsh-standard.en.md) 的实测全文相同（底座三段、
`plan:policy` 条件段、file-reference、工具 guidance 带、`ui:deliverable-file-references`），
两处不同：persona 换为上面五段；并多出 `tool:cordis` 段（order 2500）。

### `tool:cordis` 段全文（order 2500）

来源：`deepseek-harness/packages/extensions/tool-cordis/src/prompt.ts` 的
