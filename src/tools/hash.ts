 ...
 */
export async function hashFileHandler(args: Record<string, unknown>): Promise<AnyToolResult> {
  ...
  try {
    ...
    if (!stats.isFile()) {
      return fail(ErrorCode.EISDIR, `不是文件: ${filePath}`) as unknown as AnyToolResult;
    }
