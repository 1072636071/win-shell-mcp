 ...
 */
export async function netDownloadHandler(args: Record<string, unknown>): Promise<AnyToolResult> {
  ...
  try {
    ...
    if (response.body === null) {
      return fail(ErrorCode.EXEC_FAIL, `响应无 body: ${url}`) as unknown as AnyToolResult;
    }
    ...
    if (controller.signal.aborted) {
      return fail(ErrorCode.EXEC_TIMEOUT, `下载超时: ${url}`) as unknown as AnyToolResult;
    }
