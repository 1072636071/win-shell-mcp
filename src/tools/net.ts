...
function mergeHeaders(
  ...
  if (userHeaders && typeof userHeaders === "object") {
    for (const [k, v] of Object.entries(
      userHeaders as Record<string, unknown>,
    )) {
 ...
 * @returns 统一输出契约
 */
export async function netGetHandler(
...
export const netGetTool: Tool = {
  ...
  // 与同域 net_dns/net_tcp 一致归只读，可并发执行
 ...
 * @returns 统一输出契约
 */
export async function netPostHandler(
  args: Record<string, unknown>,
): Promise<AnyToolResult> {
  const url = args["url"];
...
export const netPostTool: Tool = {
  ...
  // 也应保守标 readOnlyHint: false 让宿主按独占调度，避免并发 POST 重复触发服务端动作。
 ...
 * @returns 统一输出契约
 */
export async function netDnsHandler(
  args: Record<string, unknown>,
): Promise<AnyToolResult> {
  const hostname = args["hostname"];
  const rawRecordType = args["recordType"];
  const recordType = (
    typeof rawRecordType === "string" ? rawRecordType : "A"
  ) as DnsRecordType;
...
/** net_dns 输出 schema：DNS 解析结果。 */
export const netDnsOutputSchema = z.object({
 ...
 * @returns 统一输出契约
 */
export async function netTcpHandler(
  args: Record<string, unknown>,
): Promise<AnyToolResult> {
...
/** net_tcp 输出 schema：TCP 可达性探测结果。 */
export const netTcpOutputSchema = z.object({
  reachable: z.boolean(),
  host: z.string(),
  port: z.number().int().min(0).max(65535),
