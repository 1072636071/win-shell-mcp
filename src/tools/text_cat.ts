...
import { z } from 'zod';
import { ok, fail, truncate, DEFAULT_TRUNCATE_LIMIT, type AnyToolResult } from '../contract/output.js';
 ...
 */
export async function textCatHandler(args: Record<string, unknown>): Promise<AnyToolResult> {
  ...
  try {
    ...
    const hint = encoding === 'utf8' || encoding === 'gbk' ? (encoding as string) : undefined;
