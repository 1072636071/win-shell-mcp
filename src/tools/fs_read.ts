 ...
 */
export async function fsListHandler(
...
export const fsListOutputSchema = z.object({
  entries: z
    ...
    .describe(
      "条目列表（极简为路径字符串，verbose 为含 type/size/mtime 对象）",
    ),
});
...
export const fsReadInputSchema = z.object({
  ...
  start: z
    ...
    .positive()
    .optional()
    .describe("起始行（1-indexed 含）"),
  end: z
    .number()
    .int()
    .positive()
    .optional()
    .describe("结束行（1-indexed 含）"),
  maxLen: z
    .number()
    .int()
    .positive()
    .optional()
    .describe("最大字符数（默认 2000）"),
});
 ...
 */
export async function fsReadHandler(
  args: Record<string, unknown>,
): Promise<AnyToolResult> {
  const path = args["path"];
 ...
 */
export async function fsStatHandler(
  args: Record<string, unknown>,
): Promise<AnyToolResult> {
  const path = args["path"];
