...
describe('json_get', () => {
  ...
  it('文件不存在返回 ENOENT', async () => {
    const result = await jsonGetHandler({ path: join(workDir, 'no-such.json'), expr: '.' });
    expect(isFail(result)).toBe(true);
    if (isFail(result)) {
      expect(result.error.code).toBe('ENOENT');
    }
...
describe('git_checkout', () => {
  ...
  it('paths 单独提供时还原工作区文件', async () => {
    ...
    const result = await gitCheckoutHandler({ paths: ['README.md'], cwd: workDir });
    expect(isOk(result)).toBe(true);
    if (isOk(result)) {
  ...
  it('branch + create 创建并切换到新分支', async () => {
    await initRepo(workDir);
    const result = await gitCheckoutHandler({ branch: 'feature', create: true, cwd: workDir });
    expect(isOk(result)).toBe(true);
    if (isOk(result)) {
      expect(result['checkedOut']).toBe(true);
  ...
  it('branch + paths 从指定 ref 还原文件', async () => {
    ...
    const result = await gitCheckoutHandler({ branch: 'main', paths: ['README.md'], cwd: workDir });
    expect(isOk(result)).toBe(true);
    if (isOk(result)) {
      expect(result['checkedOut']).toBe(true);
      expect(result['paths']).toEqual(['README.md']);
    }
    const restored = (await readFile(join(workDir, 'README.md'), 'utf8')).replace(/\r\n/g, '\n');
    expect(restored).toBe('# Test\n');
  });
  ...
  it('create=true 且 paths 非空返回 EINVAL', async () => {
    const result = await gitCheckoutHandler({ branch: 'feature', create: true, paths: ['a.txt'] });
    expect(isFail(result)).toBe(true);
    if (isFail(result)) {
      expect(result.error.code).toBe('EINVAL');
    }
...
describe('net_listen', () => {
  it('返回的监听条目含 port/protocol/address/pid，可选 name', async () => {
    ...
    try {
      ...
      expect(isOk(result)).toBe(true);
      if (isOk(result)) {
        const ports = (result['ports'] as Array<{
          port: number;
          protocol: string;
          address: string;
          pid: number;
  ...
  it('filter 按进程名过滤', async () => {
    ...
    try {
      ...
      expect(isOk(result)).toBe(true);
      if (isOk(result)) {
        const ports = (result['ports'] as Array<{ port: number; name?: string }>) ?? [];
  ...
  it('parseNetstatLine 解析 Windows netstat 行', () => {
    const line = '  TCP    0.0.0.0:135            0.0.0.0:0              LISTENING       1234';
    const entry = parseNetstatLine(line);
    expect(entry).toEqual({ port: 135, protocol: 'TCP', address: '0.0.0.0', pid: 1234 });
  });

  it('parseNetstatLine 忽略非 LISTENING 行', () => {
    const entry = parseNetstatLine('  TCP    0.0.0.0:135            0.0.0.0:0              ESTABLISHED     1234');
    expect(entry).toBeNull();
  });

  it('parseLsofLine 解析 lsof LISTEN 行', () => {
    const line = 'node     1234  user   6u  IPv4   0x1234      0t0  TCP *:3000 (LISTEN)';
    const entry = parseLsofLine(line);
    expect(entry).toEqual({ port: 3000, protocol: 'tcp', address: '0.0.0.0', pid: 1234, name: 'node' });
  });

  it('parseLsofLine 忽略非 LISTEN 行', () => {
    const entry = parseLsofLine('node     1234  user   6u  IPv4   0x1234      0t0  TCP 127.0.0.1:3000');
    expect(entry).toBeNull();
  });

  it('parseSsLine 解析 ss LISTEN 行', () => {
    const line = 'LISTEN 0 4096 0.0.0.0:3000 0.0.0.0:* users:(("node",pid=1234,fd=21))';
    const entry = parseSsLine(line);
    expect(entry).toEqual({ port: 3000, protocol: 'tcp', address: '0.0.0.0', pid: 1234, name: 'node' });
  });

  it('parseSsLine 忽略非 LISTEN 行', () => {
    const entry = parseSsLine('ESTAB  0 4096 0.0.0.0:3000 0.0.0.0:* users:(("node",pid=1234,fd=21))');
    expect(entry).toBeNull();
  });

  it('parseLsofLine 错误分支返回 null', () => {
    expect(parseLsofLine('node 1234 TCP *:3000 (LISTEN)')).toBeNull();
    expect(parseLsofLine('node     abc  user   6u  IPv4   0x1234      0t0  TCP *:3000 (LISTEN)')).toBeNull();
    expect(parseLsofLine('node     1234  user   6u  IPv4   0x1234      0t0  TCP (LISTEN)')).toBeNull();
    expect(parseLsofLine('node     1234  user   6u  IPv4   0x1234      0t0  TCP *:abc (LISTEN)')).toBeNull();
  });

  it('parseSsLine 错误分支返回 null', () => {
    expect(parseSsLine('LISTEN 0 4096 0.0.0.0:3000')).toBeNull();
    expect(parseSsLine('LISTEN 0 4096 0.0.0.0 0.0.0.0:* users:(("node",pid=1234,fd=21))')).toBeNull();
    expect(parseSsLine('LISTEN 0 4096 0.0.0.0:abc 0.0.0.0:* users:(("node",pid=1234,fd=21))')).toBeNull();
  ...
  it('parseNetstatLine 错误分支返回 null', () => {
    expect(parseNetstatLine('  TCP    0.0.0.0:135            0.0.0.0:0              ESTABLISHED     1234')).toBeNull();
    expect(parseNetstatLine('  TCP    0.0.0.0:135            LISTENING')).toBeNull();
    expect(parseNetstatLine('  TCP    0.0.0.0:135            0.0.0.0:0              LISTENING       abc')).toBeNull();
    expect(parseNetstatLine('  TCP    0.0.0.0                0.0.0.0:0              LISTENING       1234')).toBeNull();
    expect(parseNetstatLine('  TCP    0.0.0.0:abc            0.0.0.0:0              LISTENING       1234')).toBeNull();
  });
});
...
describe('net_download', () => {
  ...
  it('url 或 path 为空返回 EINVAL', async () => {
    const r1 = await netDownloadHandler({ url: '', path: join(workDir, 'a.txt') });
    expect(isFail(r1)).toBe(true);
    const r2 = await netDownloadHandler({ url: 'http://127.0.0.1:1/', path: '' });
