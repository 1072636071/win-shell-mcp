...
| 输出优先级链 | 交互/输出设计总纲，优先级从高到低：交互轮速最少 > LLM 请求次数最少 > 输出 token 最少 > 输入 token 最少；下游项是上游项的让步对象（ADR-0016）。轮速 = 用户输入轮次；请求 = 对 LLM 发起的 API 调用 |
| 极简 ≠ 丢信息 | 输出优先级链的约束：省 token 不得迫使 AI 再发一轮才能得到答案；极简底线 = 信息足够一轮内完成决策。宁可多输出一点，也不因省略关键字段引发追问、增加重复对话成本 |
| 极简输出 | 设计原则：默认只返回 AI 决策所需最小字段，长内容截断（默认 2000 字符）；`verbose` 参数获取完整数据（ADR-0003）。是输出优先级链第 3 项的具体化（ADR-0016） |
...
- **输出**：以四层优先级链为总纲（ADR-0016）——交互轮速最少 > LLM 请求次数最少 > 输出 token 最少 > 输入 token 最少，下游让步上游。极简、token 最小化，长输出截断 + verbose 开关取全量（ADR-0003，链第 3 项）；工具输出倾向主动充分返回判别信息，避免为确认结果再走一轮（memorial 007 原则，链第 1 项）。
